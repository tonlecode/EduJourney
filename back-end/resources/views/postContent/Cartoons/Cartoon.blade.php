...
<div class="flex-1 p-6">
    ...
    <div class="bg-white rounded-lg shadow-lg border border-gray-100 overflow-hidden">
        <div class="flex justify-between items-center p-6 border-b border-gray-100">
            ...
            <a href="/postContent/addCartoon" class="bg-cyan-500 hover:bg-cyan-600 text-white py-2 px-4 rounded-full transition-colors duration-300 flex items-center gap-2">
                    ...
                    @foreach($contents as $index => $content)
                        <tr class="hover:bg-gray-50 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4 space-x-3">
                                <a href="{{ route('cartoon.edit', $content->id) }}" class="text-cyan-600 hover:text-cyan-900 flex items-center gap-1">
                                    <i class="bi bi-pencil-square"></i><span>កែប្រែ</span>
                                </a>
                                <form action="{{ route('cartoon.destroy', $content->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
