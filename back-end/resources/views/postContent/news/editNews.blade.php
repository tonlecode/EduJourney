    ...
    <form action="{{ route('news.update', $news->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:ring-cyan-500 focus:border-cyan-500 p-2"
                placeholder="សរសេរការពិពណ៌នាអំពីសៀវភៅ...">{{ $news->description }}</textarea>
        </div>

        <div>
            <label for="file_path" class="block text-sm font-medium text-gray-700">File Path</label>
            <input type="text" name="file_path" id="file_path" value="{{ $news->file_path }}"
                class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:ring-cyan-500 focus:border-cyan-500 p-2">
        </div>

        <div>
            <label for="page_count" class="block text-sm font-medium text-gray-700">Page Count</label>
            <input type="number" name="page_count" id="page_count" value="{{ $news->page_count }}"
                class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:ring-cyan-500 focus:border-cyan-500 p-2">
        </div>

        <div>
            <label for="language" class="block text-sm font-medium text-gray-700">Language</label>
            <input type="text" name="language" id="language" value="{{ $news->language }}"
                class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:ring-cyan-500 focus:border-cyan-500 p-2">
        </div>
        ...
        <div>
            ...
            <input type="file" name="cover_image" id="cover_image" accept="image/*"
                class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:ring-cyan-500 focus:border-cyan-500 p-2">
            @if($news->cover_image)
                <img src="{{ Storage::disk('public')->url('images/' . $news->cover_image) }}" alt="Current cover image" class="mt-2 h-32">
            @endif
        </div>

        <div class="flex justify-end space-x-3">
            <a href="/postContent/news" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-full">
                ត្រឡប់ក្រោយ
            </a>
