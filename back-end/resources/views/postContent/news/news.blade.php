    ...
    <div class="bg-white rounded-lg shadow-lg border border-gray-100 overflow-hidden">
        <div class="flex justify-between items-center p-6 border-b border-gray-100">
            <div class="flex items-center gap-3">
                ...
                <h2 class="text-xl font-semibold text-gray-800">បញ្ជី ព័ត៌មាន</h2>
            </div>
            <a href="/postContent/addNews" class="bg-cyan-500 hover:bg-cyan-600 text-white py-2 px-4 rounded-full transition-colors duration-300 flex items-center gap-2">
                <i class="bi bi-person-plus"></i>
                <span>បន្ថែមព័ត៌មាន</span>
            </a>
        </div>
            ...
            <table class="min-w-full table-auto">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">តំណភ្ជាប់ news</th>
                    ...
                    @foreach($contents as $index => $content)
                        <tr class="hover:bg-gray-50 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4 text-sm">
                                <a href="{{ asset($content->path) }}" target="_blank" class="text-blue-600 hover:underline">ពិនិត្យ news</a>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 space-x-3">
                                <a href="{{ route('news.edit', $content->id) }}" class="text-cyan-600 hover:text-cyan-900 flex items-center gap-1">
                                    <i class="bi bi-pencil-square"></i><span>កែប្រែ</span>
                                </a>
                                <form action="{{ route('news.destroy', $content->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
